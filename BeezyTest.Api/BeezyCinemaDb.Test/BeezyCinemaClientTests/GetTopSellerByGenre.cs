...
using BeezyTest.BeezyCinemaDb;
using BeezyTest.BeezyCinemaDb.Models;
using BeezyTest.BeezyCinemaDb.Models.BeezyCinema;

namespace BeezyCinemaDb.Test.BeezyCinemaClientTests
{
	public class GetTopSellerByGenre
	{
		...
		public void SetupData()
		{
			DbContextOptions = new DbContextOptionsBuilder<BeezyCinemaContext>()
				.UseInMemoryDatabase(databaseName: "GetTopSellerByGenre")
				.Options;
		...
		private void LoadTestData(BeezyCinemaContext context)
		{
			...
			context.Genre.Add(new Genre { Id = 5, Name = "Genre Five" }); //This genre isn't connected to any movie
			...
			context.Session.Add(new BeezyTest.BeezyCinemaDb.Models.BeezyCinema.Session { Id = 24, MovieId = 3, RoomId = 13, SeatsSold = 100 });
			context.Session.Add(new BeezyTest.BeezyCinemaDb.Models.BeezyCinema.Session { Id = 25, MovieId = 3, RoomId = 16, SeatsSold = 100 });
			context.Session.Add(new BeezyTest.BeezyCinemaDb.Models.BeezyCinema.Session { Id = 26, MovieId = 3, RoomId = 19, SeatsSold = 100 });
			...
			context.Session.Add(new BeezyTest.BeezyCinemaDb.Models.BeezyCinema.Session { Id = 34, MovieId = 4, RoomId = 12, SeatsSold = 90 });
			context.Session.Add(new BeezyTest.BeezyCinemaDb.Models.BeezyCinema.Session { Id = 35, MovieId = 4, RoomId = 15, SeatsSold = 90 });
			context.Session.Add(new BeezyTest.BeezyCinemaDb.Models.BeezyCinema.Session { Id = 36, MovieId = 4, RoomId = 18, SeatsSold = 90 });
		...
		/// <summary>
		/// Straightforward test, find the highest grossing movie in city 1, which is of genre 1.
		/// This is the same as the top grossing movie in the entire city.
		/// </summary>
		/// <returns></returns>
		[Test]
		public async Task Test1()
		{
			Assert.AreEqual("Movie One", await TestObject.GetTopSellerByGenre(1, "Genre One"));
		}
		/// <summary>
		/// Find the highest grossing movie in city 1, which is of genre 1.
		/// The top grossing movie in city 2 is movie 2, however, this is not of genre 3.
		/// Therefore the correct result here is movie 3.
		/// </summary>
		/// <returns></returns>
		[Test]
		public async Task Test2()
		{
			Assert.AreEqual("Movie Three", await TestObject.GetTopSellerByGenre(2, "Genre Three"));
		}

		/// <summary>
		/// Find the highest grossing movie in city 3, which is of genre 5.
		/// No movie of genre 5 is played in city three, therefore the result returns null.
		/// </summary>
		/// <returns></returns>
		[Test]
		public async Task Test3()
		{
			Assert.AreEqual(null, await TestObject.GetTopSellerByGenre(3, "Genre Five"));
		}
	}
}